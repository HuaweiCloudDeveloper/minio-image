...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "9001",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },       
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "MinIO20250422221226.0.0-1-HCE2.0" ? local.instance_image_id_maps_v1[data.huaweicloud_availability_zones.az.region] : (var.Version == "MinIO20250422221226.0.0-1-Ubuntu24.04" ? local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] )
